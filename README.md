<!-- vscode-markdown-toc -->
* 1. [Iframe feature](#Iframefeature)
	* 1.1. [Open links in an iframe in the current page](#Openlinksinaniframeinthecurrentpage)
	* 1.2. [iframe warnings](#iframewarnings)
	* 1.3. [load local pages inside the iframe](#loadlocalpagesinsidetheiframe)
* 2. [Sound Audio Features](#SoundAudioFeatures)
	* 2.1. [mute button](#mutebutton)
	* 2.2. [mute sound if player goes AFK](#mutesoundifplayergoesAFK)
* 3. [Sections and surveys feature](#Sectionsandsurveysfeature)
	* 3.1. [show section](#showsection)
	* 3.2. [add survey](#addsurvey)
* 4. [Traffic log features](#Trafficlogfeatures)
	* 4.1. [Traffic log](#Trafficlog)
	* 4.2. [Traffic resume by email](#Trafficresumebyemail)
* 5. [Graphic features](#Graphicfeatures)
	* 5.1. [different resolution background images](#differentresolutionbackgroundimages)
	* 5.2. [force scale and offset](#forcescaleandoffset)
	* 5.3. [perspective mode](#perspectivemode)
	* 5.4. [animated background](#animatedbackground)
* 6. [Other improvements](#Otherimprovements)
	* 6.1. [Join and Leave functions for any room](#JoinandLeavefunctionsforanyroom)
	* 6.2. [Define actions on NPC clicks](#DefineactionsonNPCclicks)
	* 6.3. [Define actions in text commands](#Defineactionsintextcommands)
	* 6.4. [definepre and post action](#definepreandpostaction)
	* 6.5. [Debug more accesible](#Debugmoreaccesible)
...
##  1. <a name='Iframefeature'></a>Iframe feature

###  1.1. <a name='Openlinksinaniframeinthecurrentpage'></a>Open links in an iframe in the current page
...
###  1.2. <a name='iframewarnings'></a>iframe warnings

Be careful to check if the url selected allow iframes. 

###  1.3. <a name='loadlocalpagesinsidetheiframe'></a>load local pages inside the iframe

You can put html pages in the public folder and load them inside the iframe.

##  2. <a name='SoundAudioFeatures'></a>Sound Audio Features

Some basic sound features were added.

###  2.1. <a name='mutebutton'></a>mute button

A button to switch on/off the audio.

###  2.2. <a name='mutesoundifplayergoesAFK'></a>mute sound if player goes AFK
...
##  3. <a name='Sectionsandsurveysfeature'></a>Sections and surveys feature

###  3.1. <a name='showsection'></a>show section

Parrafo

###  3.2. <a name='addsurvey'></a>add survey
...
##  4. <a name='Trafficlogfeatures'></a>Traffic log features

###  4.1. <a name='Trafficlog'></a>Traffic log
...
Log files will be created in /logs.  It will register the following actions in a csv file.
...
* open ifrane:
    ...
    ```
    timeStamp (ISO format), userID, 'openIframe', url
    ```

* pool answers:

    ```
    timeStamp (ISO format), userID, 'poolAnswers', poolData...
    ```

###  4.2. <a name='Trafficresumebyemail'></a>Traffic resume by email
...
##  5. <a name='Graphicfeatures'></a>Graphic features

###  5.1. <a name='differentresolutionbackgroundimages'></a>different resolution background images
...
###  5.2. <a name='forcescaleandoffset'></a>force scale and offset
...
###  5.3. <a name='perspectivemode'></a>perspective mode
...
###  5.4. <a name='animatedbackground'></a>animated background
...
##  6. <a name='Otherimprovements'></a>Other improvements

###  6.1. <a name='JoinandLeavefunctionsforanyroom'></a>Join and Leave functions for any room
...
###  6.2. <a name='DefineactionsonNPCclicks'></a>Define actions on NPC clicks
...
###  6.3. <a name='Defineactionsintextcommands'></a>Define actions in text commands
...
###  6.4. <a name='definepreandpostaction'></a>definepre and post action
...
###  6.5. <a name='Debugmoreaccesible'></a>Debug more accesible
